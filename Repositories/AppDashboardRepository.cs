...
public class AppDashboardRepository : IAppDashboardRepository
{
    ...
    public async Task<List<MusicTrack>> GetAllUserTracks() {
        var currentUser = _httpContextAccesser
            .HttpContext
            ?.User
            .GetUserId();
        var userTracks = _context.Tracks
            .Where(
                track => track.AppUser.Id == currentUser
            );
    ...
    public async Task<List<MusicConcert>> GetAllUserConcerts() {
        var currentUser = _httpContextAccesser
            ...
            ?.User
            .GetUserId();
        var userConcerts = _context.Concerts
            .Where(track => track.AppUser.Id == currentUser);
